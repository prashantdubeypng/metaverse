...
export default function CreateSpacePage() {
  ...
  const handleCreateSpace = async () => {
    ...
    try {
      ...
      let body: { name: string; dimensions: string; mapId?: string };
      if (selectedMapId) {
        const selectedMap = maps.find((m) => m.id === selectedMapId);
        const dimensions = selectedMap ? `${selectedMap.width}x${selectedMap.height}` : `${customWidth}x${customHeight}`;
        body = {
          name: spaceName.trim(),
          dimensions,
          mapId: selectedMapId,
    ...
    return (
      <div className="max-w-xl mx-auto mt-16 p-8 bg-white rounded shadow">
        ...
        <div className="bg-gray-100 rounded px-4 py-2 mb-4 font-mono">
          {typeof window !== "undefined" ? `${window.location.origin}/space/${createdSpaceId}` : `/space/${createdSpaceId}`}
        </div>
        <button
